...
void compileForSt(void) {
  ...
  eat(SB_ASSIGN);
  exprType1 = compileExpression();
  checkIntType(exprType1);

  eat(KW_TO);
  exprType2 = compileExpression();
  checkIntType(exprType2);
...
void compileArgument(Object* param) {
  ...
  if (param->paramAttrs->kind == PARAM_REFERENCE) {
    if (lookAhead->tokenType == TK_IDENT) {
      ...
      if (lookAhead->tokenType == SB_LPAR) {
        ...
        argType = obj->funcAttrs->returnType;
      } else {
        if (lookAhead->tokenType == SB_PLUS || lookAhead->tokenType == SB_MINUS ||
          ...
          switch (obj->kind) {
          case OBJ_VARIABLE:
            argType = obj->varAttrs->type;
            if (argType->typeClass == TP_ARRAY)
              argType = compileIndexes(argType);
            break;
          case OBJ_PARAMETER:
            argType = obj->paramAttrs->type;
            break;
          case OBJ_FUNCTION:
            argType = obj->funcAttrs->returnType;
            break;
  ...
  if (argType != NULL && param->paramAttrs->type != NULL) {
    if (compareType(argType, param->paramAttrs->type) == 0) {
      error(ERR_TYPE_INCONSISTENCY, currentToken->lineNo, currentToken->colNo);
    }
  }
}
...
void compileArguments(ObjectNode* paramList) {
  ...
  case SB_LPAR:
    ...
    eat(SB_RPAR);
    break;
    // Check FOLLOW set - no arguments provided
  case SB_TIMES:
  case SB_SLASH:
  case SB_PLUS:
  case SB_MINUS:
...
void compileExpression3(Type* firstTermType) {
  Type* type;

  switch (lookAhead->tokenType) {
  case SB_PLUS:
    eat(SB_PLUS);
    checkIntType(firstTermType);
    type = compileTerm();
    checkIntType(type);
    compileExpression3(type);
    break;
...
void compileTerm2(Type* firstFactorType) {
  Type* type;

  switch (lookAhead->tokenType) {
  case SB_TIMES:
    eat(SB_TIMES);
    checkIntType(firstFactorType);
    type = compileFactor();
    checkIntType(type);
    compileTerm2(type);
    break;
