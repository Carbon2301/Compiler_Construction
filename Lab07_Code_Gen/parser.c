...
void compileForSt(void) {
  ...
  // Parse and save loop variable info
  eat(TK_IDENT);
  loopVar = checkDeclaredLValueIdent(currentToken->string);
  varType = loopVar->varAttrs->type;

  // Initialize: var := start
  genVariableAddress(loopVar);  // push address of loop variable
  eat(SB_ASSIGN);
  type = compileExpression();  // evaluate start value
  ...
  genVariableValue(loopVar);  // load variable value
  eat(KW_TO);
  type = compileExpression();  // evaluate end value
  checkTypeEquality(varType, type);
  genLE();  // check var <= end
  fjLabel = genFJ(DC_VALUE);  // if false, exit loop
  ...
  // Increment: var := var + 1
  genVariableAddress(loopVar);  // push address of var
  genVariableValue(loopVar);  // load current value
  genLC(1);  // push 1
  genAD();  // add
  genST();  // store back to var
  
  genJ(beginLoop);  // jump back to loop start
  updateFJ(fjLabel, getCurrentCodeAddress());
}
