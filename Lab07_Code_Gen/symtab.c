...
void initSymTab(void) {
	...
	addObject(&(symtab->globalObjectList), readcFunction);

	readiFunction = createFunctionObject("READI");
	readiFunction->funcAttrs->returnType = makeIntType();
	addObject(&(symtab->globalObjectList), readiFunction);

	writeiProcedure = createProcedureObject("WRITEI");
	param = createParameterObject("i", PARAM_VALUE);
	param->paramAttrs->type = makeIntType();
	addObject(&(writeiProcedure->procAttrs->paramList), param);
	addObject(&(symtab->globalObjectList), writeiProcedure);

	writecProcedure = createProcedureObject("WRITEC");
	param = createParameterObject("ch", PARAM_VALUE);
	param->paramAttrs->type = makeCharType();
	addObject(&(writecProcedure->procAttrs->paramList), param);
	addObject(&(symtab->globalObjectList), writecProcedure);

	writelnProcedure = createProcedureObject("WRITELN");
	addObject(&(symtab->globalObjectList), writelnProcedure);
