...
void compileProgram(void) {
  ...
  eat(KW_PROGRAM);
  char programName[MAX_IDENT_LEN + 1];
  strcpy(programName, lookAhead->string);
  eat(TK_IDENT);
  symtab->program = createProgramObject(programName);
...
void compileBlock(void) {
  ...
  if (lookAhead->tokenType == KW_CONST) {
    ...
    do {
      ...
      // Create and declare constant object
...
void compileFuncDecl(void) {
  ...
  compileParams();
  eat(SB_COLON);
  Type* returnType = compileType();  // Parse return type
...
ConstantValue* compileConstant(void) {
  ...
  case SB_MINUS:
    eat(SB_MINUS);
    {
      ConstantValue* temp = compileConstant2();
      if (temp->type == TP_INT) {
        constValue = makeIntConstant(-temp->intValue);
        free(temp);
      } else {
        constValue = temp;
      }
  ...
  case TK_CHAR:
    {
      char charValue = lookAhead->string[0];
      eat(TK_CHAR);
      constValue = makeCharConstant(charValue);
    }
    break;
  default:
    constValue = compileConstant2();
...
ConstantValue* compileConstant2(void) {
  ...
  case TK_NUMBER:
    {
      int numValue = lookAhead->value;
      eat(TK_NUMBER);
      constValue = makeIntConstant(numValue);
    }
    break;
  case TK_IDENT:
    {
