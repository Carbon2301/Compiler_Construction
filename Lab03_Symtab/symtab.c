...
void freeObject(Object* obj) {
  ...
  case OBJ_CONSTANT:
    ...
    break;
  case OBJ_TYPE:
    if (obj->typeAttrs->actualType != intType && obj->typeAttrs->actualType != charType) {
      freeType(obj->typeAttrs->actualType);
    }
    free(obj->typeAttrs);
    break;
  case OBJ_VARIABLE:
    if (obj->varAttrs->type != intType && obj->varAttrs->type != charType) {
  ...
  case OBJ_FUNCTION:
    ...
    if (obj->funcAttrs->returnType != intType && obj->funcAttrs->returnType != charType) {
  ...
  case OBJ_PROGRAM:
    ...
    break;
  case OBJ_PARAMETER:
    if (obj->paramAttrs->type != intType && obj->paramAttrs->type != charType) {
